...
const AR2List = () => {
  ...
  const GetData = useCallback(() => {
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        setMyData(data);
        if (data.length === 0) {
          setMessage(<MyMessage key={Date.now()}messageText="Tiada rekod AR2 untuk sesi ini." messagecolor="orange" />);
        }
      })
      .catch((err) => {
        setMessage(<MyMessage key={Date.now()}messageText={parseApiError(err, "Gagal memuatkan data AR2.")} messagecolor="red" />);
        setMyData([]);
      })
  ...
  const handleOpenEdit = (row) => {
    setEditRow(row);
    setDialogError("");
    setEditMark(row.AR2grade === "TH" ? "TH" : (row.AR2mark ?? ""));
    setEditGrade(row.AR2grade ?? "");
    setEditOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await axiosInstance.patch(`/ar2/${editRow.headcountID}/`, {
        AR2mark:
          editMark === "" || editMark === "TH" ? null : Number(editMark),
        AR2grade: editGrade === "" ? null : editGrade,
      });
      ...
      setMessage(
        <MyMessage
          key={Date.now()}
          messageText="AR2 berjaya dikemaskini!"
          messagecolor="green"
        />,
      );
    } catch (err) {
      setDialogError(parseApiError(err, "Ralat semasa mengemaskini AR2."));
    } finally {
      setSaving(false);
    }
  };

  // ── Clear AR2 for this specific student-subject ────────────────────────────
  const handleDeleteConfirm = async () => {
    if (!deleteRow || !deleteRow.headcountID) return;
    
    setDeleting(true);
    try {
      await axiosInstance.patch(`/ar2/${deleteRow.headcountID}/`, {
        AR2mark: null,
        AR2grade: null,
      });
      ...
      setMessage(
        <MyMessage
          key={Date.now()}
          messageText="AR2 berjaya dipadamkan!"
          messagecolor="green"
        />,
      );
    } catch (err) {
      setDialogError(parseApiError(err, "Ralat semasa memadam AR2."));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      Cell: ({ cell, row }) => {
        ...
        const grade = row.original.AR2grade;
  ...
  return (
    <Box>
      <PageHeader icon={<AssignmentIcon sx={{ color: "primary.main" }} />} title="Peperiksaan AR2">
      ...
      <BulkUploadDialog
        ...
        title="Muat Naik Markah AR2"
        uploadUrl="/ar2/upload-marks/"
        downloadUrl="/ar2/download-marks-template/"
        downloadFileName="template_ar2_markah.xlsx"
        instructions={[
        ...
        onSuccess={() => {
          GetData();
          setMessage(<MyMessage key={Date.now()}messageText="Markah AR2 berjaya dimuat naik!" messagecolor="green" />);
        }}
      />
      ...
      >
        <Typography>
          Adakah anda pasti ingin memadam markah AR2 ini? Tindakan ini tidak boleh dibuat asal.
        </Typography>
        {deleteRow && (
