...
const PrintHeadcountSlip = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const year = useYear();
  const { studentIds = [] } = location.state || {};
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axiosInstance.post('/headcount-slip/bulk/', { ids: studentIds })
      .then(res => setSlipsData(res.data.data))
      .catch(() => {})
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center" }}>
            ...
            <Typography sx={{ marginLeft: "15px", fontWeight: "bold" }} variant="h6">
              Slip Headcount - Cetak ({slipsData.length})
            </Typography>
          </Box>
          <Box sx={{ display: "flex", gap: 1 }}>
            <BackButton onClick={() => navigate(`/${year}/headcount-slip`)} />
            <PrimaryButton startIcon={<PrintIcon />} onClick={handlePrint} sx={{ px: 3 }}>
              Cetak Slip
            </PrimaryButton>
          </Box>
        </Box>
        ...
        {slipsData.map((slipData, index) => (
          <Box
            ...
            sx={{
              ...
              '@media print': { margin: 0, padding: 0, boxShadow: 'none', pageBreakAfter: index < slipsData.length - 1 ? "always" : "auto" },
            }}
          >
            <Paper
              elevation={0}
...
export default PrintHeadcountSlip;
