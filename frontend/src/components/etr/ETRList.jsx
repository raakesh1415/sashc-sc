...
const ETRList = () => {
  ...
  const GetData = useCallback(() => {
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        setMyData(data);
        if (data.length === 0) {
          setMessage(<MyMessage key={Date.now()}messageText="Tiada rekod ETR untuk sesi ini." messagecolor="orange" />);
        }
      })
      .catch((err) => {
        setMessage(<MyMessage key={Date.now()}messageText={parseApiError(err, "Gagal memuatkan data ETR.")} messagecolor="red" />);
        setMyData([]);
      })
  ...
  const handleOpenEdit = (row) => {
    setEditRow(row);
    setDialogError("");
    setEditMark(row.ETRgrade === "TH" ? "TH" : (row.ETRmark ?? ""));
    setEditGrade(row.ETRgrade ?? "");
    setEditOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await axiosInstance.patch(`/etr/${editRow.headcountID}/`, {
        ETRmark:
          editMark === "" || editMark === "TH" ? null : Number(editMark),
        ETRgrade: editGrade === "" ? null : editGrade,
      });
      ...
      setMessage(
        <MyMessage
          key={Date.now()}
          messageText="ETR berjaya dikemaskini!"
          messagecolor="green"
        />,
      );
    } catch (err) {
      setDialogError(parseApiError(err, "Ralat semasa mengemaskini ETR."));
    } finally {
      setSaving(false);
    }
  };

  // ── Clear ETR for this specific student-subject ────────────────────────────
  const handleDeleteConfirm = async () => {
    if (!deleteRow || !deleteRow.headcountID) return;
    
    setDeleting(true);
    try {
      await axiosInstance.patch(`/etr/${deleteRow.headcountID}/`, {
        ETRmark: null,
        ETRgrade: null,
      });
      ...
      setMessage(
        <MyMessage
          key={Date.now()}
          messageText="ETR berjaya dipadamkan!"
          messagecolor="green"
        />,
      );
    } catch (err) {
      setDialogError(parseApiError(err, "Ralat semasa memadam ETR."));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      Cell: ({ cell, row }) => {
        ...
        const grade = row.original.ETRgrade;
  ...
  return (
    <Box>
      <PageHeader icon={<AssignmentIcon sx={{ color: "primary.main" }} />} title="Peperiksaan ETR">
      ...
      <BulkUploadDialog
        ...
        title="Muat Naik Markah ETR"
        uploadUrl="/etr/upload-marks/"
        downloadUrl="/etr/download-marks-template/"
        downloadFileName="template_etr_markah.xlsx"
        instructions={[
        ...
        onSuccess={() => {
          GetData();
          setMessage(<MyMessage key={Date.now()}messageText="Markah ETR berjaya dimuat naik!" messagecolor="green" />);
        }}
      />
      ...
      >
        <Typography>
          Adakah anda pasti ingin memadam markah ETR ini? Tindakan ini tidak boleh dibuat asal.
        </Typography>
        {deleteRow && (
